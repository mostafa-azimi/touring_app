...
export class ShipHeroOrderService {
  ...
  /**
   * Create a standalone purchase order with "ShipHero Tours" as the fulfillment status
   */
  async createStandalonePurchaseOrder(tourId: string): Promise<{
    success: boolean;
    message: string;
    poNumber?: string;
    errors: string[];
    ...
    try {
      ...
      if (!warehouse?.shiphero_warehouse_id) {
        return {
          success: false,
          message: 'Warehouse not configured for ShipHero',
          errors: ['Missing warehouse ShipHero configuration']
        }
      }

      // Get swag items for the purchase order
      const { data: swagItems, error: swagError } = await this.supabase
        .from('swag_items')
        .select('*')
        .eq('active', true)

      if (swagError) {
        console.error('Error fetching swag items:', swagError)
        return {
          success: false,
          message: 'Failed to fetch swag items',
          errors: ['Database error fetching swag items']
        }
      }
      ...
      if (!swagItems || swagItems.length === 0) {
        return {
          success: false,
          message: 'No active swag items found',
          errors: ['No swag items configured']
        }
      }

      // Get participant count for quantity calculation
      const { data: participants, error: participantsError } = await this.supabase
        .from('tour_participants')
        .select('id')
        .eq('tour_id', tourId)
      ...
      if (participantsError) {
        ...
        return {
          success: false,
          message: 'Failed to fetch participants',
          errors: ['Database error fetching participants']
        }
      }

      const participantCount = (participants?.length || 0) + (tour.host ? 1 : 0) // Include host

      if (participantCount === 0) {
        return {
          success: false,
          message: 'No participants found for tour',
          errors: ['Tour has no participants']
        }
      }

      // Create line items for purchase order
      const lineItems = swagItems.map(itemData => ({
        sku: itemData.sku,
        quantity: participantCount, // One per participant + host
      ...
      if (purchaseOrderData.data?.purchase_order_create?.purchase_order) {
        const purchaseOrder = purchaseOrderData.data.purchase_order_create.purchase_order
        console.log(`Created standalone purchase order: ${purchaseOrder.po_number} (ID: ${purchaseOrder.id})`)
        console.log('🔍 Standalone PO Response data:', {
          id: purchaseOrder.id,
          legacy_id: purchaseOrder.legacy_id,
          po_number: purchaseOrder.po_number
        })
        ...
        // Store ShipHero purchase order details in database
        const shipheroPOUrl = `https://app.shiphero.com/dashboard/purchase-orders/details/${purchaseOrder.legacy_id}`
        console.log('🔗 Generated Standalone PO URL:', shipheroPOUrl)
        const { error: updateError } = await this.supabase
          .from('tours')
          .update({
            shiphero_purchase_order_id: purchaseOrder.id,
        ...
        console.log(`Successfully stored standalone purchase order tracking info for tour ${tourId}`)
        return {
          success: true,
          message: `Created purchase order "${purchaseOrder.po_number}"`,
          poNumber: purchaseOrder.po_number,
        ...
        const errorMsg = purchaseOrderData.errors?.[0]?.message || 'Unknown error creating purchase order'
        console.error('ShipHero purchase order creation failed:', purchaseOrderData)
        return {
          success: false,
          message: errorMsg,
          errors: [errorMsg]
      ...
      console.error('Error creating standalone purchase order:', error)
      return {
        success: false,
        message: 'Failed to create standalone purchase order',
